...
import { useLoginUser } from "../../services/auth/login-auth";
import { redirect, useNavigate } from "react-router-dom";
export const LoginPage = () => {
  // const navigate = useNavigate();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const { mutate: sendLoginUser, isSuccess } = useLoginUser();
  ...
  // buat pindah ke dashboard
  // if (isSuccess) {
  //   navigaet("/dashboard")
  // }
  return (
    <div>
      <div className="relative mx-4 flex min-h-screen flex-col justify-center overflow-hidden bg-gslate-900 py-6 sm:py-12">
        ...
        <button
          className="ml-4 mt-4 w-28 rounded-lg bg-red-500 text-white"
          onClick={() => {
            loginUser();
          }}
        >
